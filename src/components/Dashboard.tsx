...
const EmployeeCard: React.FC<{ employee: Employee; onEdit: () => void; onDelete: () => void }> = ({ employee, onEdit, onDelete }) => (
    <div className="bg-white p-4 rounded-lg shadow-sm flex items-center space-x-4">
        <img src={employee.imageUrl} alt={employee.name} className="w-16 h-16 rounded-full object-cover" />
        <div className="flex-1">
            <h3 className="font-bold text-lg text-slate-800">{employee.name}</h3>
            <p className="text-sm text-slate-500">{employee.position}</p>
            <div className="mt-1">
                <p className="text-md font-semibold text-emerald-600">{formatCurrency(employee.dailyRate)} / hari</p>
                <p className="text-sm text-slate-500">Tunjangan: {formatCurrency(employee.weeklyAllowance)} / minggu</p>
            </div>
        </div>
        <div className="flex flex-col space-y-2">
            <button onClick={onEdit} className="p-2 text-slate-500 hover:text-indigo-600 hover:bg-slate-100 rounded-full transition-colors" aria-label={`Edit ${employee.name}`}>
                <PencilIcon />
            </button>
            <button onClick={onDelete} className="p-2 text-slate-500 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors" aria-label={`Hapus ${employee.name}`}>
                <TrashIcon />
            </button>
        </div>
    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm flex items-center space-x-3">
                        <div className="bg-emerald-100 p-3 rounded-full">
                           <CurrencyDollarIcon className="text-emerald-600" />
                        </div>
                        <div>
