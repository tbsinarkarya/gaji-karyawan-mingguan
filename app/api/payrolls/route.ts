...
export async function POST(req: Request) {
  try {
    ...
    for (const p of employeePayments) {
      ...
      const extraAllowance = Number(p.totalAllowance || 0);
    ...
    return new NextResponse(JSON.stringify(weekly), { status: 201, headers: { "Content-Type": "application/json" } });
  } catch (error) {
...
export async function DELETE(req: Request) {
  try {
    ...
    if (!id) {
      return new NextResponse("Payroll id or week identifier required", { status: 400 });
    }

    // jika id numeric → hapus row payroll dengan id
    if (!Number.isNaN(Number(id))) {
      const delRes = await pool.query(`DELETE FROM payroll WHERE id = $1 RETURNING *`, [Number(id)]);
      if (delRes.rows.length === 0) return new NextResponse("Payroll not found", { status: 404 });
    ...
    if (typeof id === "string" && id.includes("_")) {
      ...
      const delRes = await pool.query(`DELETE FROM payroll WHERE week_start = $1::date AND week_end = $2::date RETURNING *`, [ws, we]);
