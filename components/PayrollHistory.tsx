...
const { useState: useStateHistory, useMemo: useMemoHistory } = React;
...
const formatCurrencyHistory = (amount: number) => {
...
const handlePrint = (payrollId: string) => {
    ...
    printDocument.open();
    printDocument.write('<html><head><title>Cetak Slip Gaji</title></head><body></body></html>');
    printDocument.close();

    // Append TailwindCSS script to the iframe's head
    const tailwindScript = printDocument.createElement('script');
    tailwindScript.src = 'https://cdn.tailwindcss.com';
    printDocument.head.appendChild(tailwindScript);
    
    // Append a custom style tag for print-specific adjustments
    ...
    // Append the content to be printed
    ...
    // Wait for Tailwind script to load and apply styles, then print
    tailwindScript.onload = () => {
        // A small extra delay can help ensure styles are fully rendered
        setTimeout(() => {
            if (iframe.contentWindow) {
                iframe.contentWindow.focus(); // Required for some browsers
                iframe.contentWindow.print();
            }
            // Clean up the iframe after printing dialog is likely closed
            setTimeout(() => {
                document.body.removeChild(iframe);
            }, 1000);
        }, 500); // 500ms delay for JIT compiler of Tailwind
    };

    // Fallback in case onload fails for some reason
    tailwindScript.onerror = () => {
        console.error("Failed to load Tailwind CSS script for printing.");
        alert("Gagal memuat gaya cetak. Hasil cetak mungkin tidak sesuai.");
        document.body.removeChild(iframe);
    };
};
...
const handleShareWhatsApp = (payroll: WeeklyPayroll) => {
    ...
    message += `*Total Gaji Dibayarkan:* ${formatCurrencyHistory(payroll.totalPayroll)}\n\n`;
    ...
    payroll.employeePayments.forEach(p => {
        ...
        message += `*Gaji Pokok:* ${formatCurrencyHistory(p.basePay)}\n`;
        message += `*Tunjangan:* ${formatCurrencyHistory(p.totalAllowance)}\n`;
        if (p.loanDeduction > 0) {
            message += `*Potongan:* -${formatCurrencyHistory(p.loanDeduction)}\n`;
        }
        message += `*Total Diterima:* *${formatCurrencyHistory(p.totalPay)}*\n`;
        message += `-----------------------------------\n`;
    });
        ...
        <div className="flex justify-between items-start mb-3">
            <div>
                ...
                <div className="flex items-center space-x-2 font-bold text-lg text-brand-secondary mt-1">
                    ...
                    <span>{formatCurrencyHistory(payroll.totalPayroll)}</span>
                </div>
            </div>
                ...
                <div key={payment.employeeId} className="flex justify-between items-start text-sm py-2 border-b border-slate-100 last:border-b-0">
                    <div className="flex-1">
                        ...
                        <div className="text-xs text-slate-600 mt-1 pl-2 border-l-2 border-slate-200 space-y-0.5">
                            <div className="flex justify-between"><span>Gaji Pokok:</span> <span>{formatCurrencyHistory(payment.basePay)}</span></div>
                            <div className="flex justify-between"><span>Tunjangan:</span> <span>{formatCurrencyHistory(payment.totalAllowance)}</span></div>
                            {payment.loanDeduction > 0 && (
                                <div className="flex justify-between text-red-600"><span>Potongan:</span> <span>-{formatCurrencyHistory(payment.loanDeduction)}</span></div>
                            )}
                        </div>
                    </div>
                    <p className="font-bold text-slate-800 ml-4">{formatCurrencyHistory(payment.totalPay)}</p>
                </div>
            ))}
        </div>
    </div>
...
const MonthlySummaryCard: React.FC<{
    ...
    const [isExpanded, setIsExpanded] = useStateHistory(false);
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
            ...
            >
                ...
                <div className="flex items-center space-x-4">
                    <p className="font-bold text-lg text-brand-secondary">{formatCurrencyHistory(totalPayroll)}</p>
...
const PayrollHistory: React.FC<PayrollHistoryProps> = ({ payrolls, onDeletePayroll }) => {
    const [view, setView] = useStateHistory<'weekly' | 'monthly'>('weekly');

    const monthlyData = useMemoHistory(() => {
        const groupedByMonth: Record<string, { totalPayroll: number; weeklyPayrolls: WeeklyPayroll[] }> = {};

        payrolls.forEach(payroll => {
            const monthYear = new Date(payroll.weekStartDate).toLocaleString('id-ID', { month: 'long', year: 'numeric' });
